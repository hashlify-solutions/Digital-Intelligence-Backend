...
top_card_data_pipeline=[
        ...
        '$facet': {
                    ...
                    '$addFields': {
                        'NormalizedApplication': {
                            '$switch': {
                                'branches': [
                                    {
                                        'case': { '$eq': ['$Message Type', 'location'] },
                                        'then': 'Location'
                                    },
                                    {
                                        'case': { '$eq': ['$Message Type', 'browsing_history'] },
                                        'then': 'Browsing History'
                                    },
                                    {
                                        'case': { '$eq': ['$Message Type', 'note'] },
                                        'then': 'Note'
                                    }
                                ],
                                'default': { '$ifNull': ['$Application', 'Unknown'] }
                            }
                        }
                    }
                },
            ...
            'message_types_count': [
                ...
                },
                {
                    '$project': {
                        '_id': 0,
                        'message_types': { '$arrayToObject': '$message_types' }
                    }
                }
            ],
    ...
    },
    {
        '$project': {
            ...
            # 'locations_count': { '$arrayElemAt': ['$main_metrics.locations_count', 0] },
            # 'notes_count': { '$arrayElemAt': ['$main_metrics.notes_count', 0] },
            'top_3_applications_message_count': {
                '$let': {
                    'vars': {
                        'base': { '$ifNull': [ { '$arrayElemAt': ['$message_types_count.message_types', 0] }, {} ] }
                    },
                    'in': {
                        '$mergeObjects': [
                            { 'note': { '$ifNull': ['$$base.note', 0] } },
                            { 'location': { '$ifNull': ['$$base.location', 0] } },
                            '$$base'
                        ]
                    }
                }
